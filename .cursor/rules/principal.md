...
- Better Auth:
  - Usa siempre los métodos/utilidades de better-auth para login, logout, sesión, y lectura de usuario.
  - No re-implementar flujos cubiertos por better-auth; preferir sus helpers antes que lógica ad-hoc.
  - Para páginas y server actions, obtener el usuario/sesión con better-auth.

- NextResponse (App Router API):
  - Para endpoints en `app/api/**`, responde con `NextResponse.json(...)`, maneja errores y códigos HTTP.
  - En server actions/redirecciones, usa `NextResponse` en lugar de `res`.

## UI

- Siempre usar componentes de `shadcn/ui` (botones, inputs, diálogos, etc.).
- Formularios con `react-hook-form` + `@/components/ui/form`.
- Navegación con `next/link` y botones de shadcn.
- Notificaciones con `sonner` (no usar `toast` deprecado).
